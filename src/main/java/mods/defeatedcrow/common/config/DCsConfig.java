...
public class DCsConfig {
	...
	public void config(Configuration cfg) {
		...
		try {
			...
			cfg.addCustomCategoryComment("world setting", "This setting is for world generation.");

			cfg.addCustomCategoryComment("entity setting", "This setting is for entities of this mod.");

			cfg.addCustomCategoryComment("setting", "This setting is for some system of this mod.");
			...
			Property allowSlimeBall = cfg.get("setting", "Allow Slimeball OreDic", allowSlimeBallDic,
					"Allow to add SlimeBall and Animalglue to Oredictionary.");
			Property noSteam = cfg.get("render setting", "Not Render Foods Steam", noRenderFoodsSteam,
					"Not Render Steam on foods of this mod.");
			Property noFirestarter = cfg.get("setting", "Disable Fierstarter", disableFireSteater,
					"Disable recipe for crafting firestarter.");
			Property noClam = cfg.get("world setting", "Disable Clams", disableClam,
					"Not generating clams on overworld.");
			Property nowetGC = cfg.get("Setting", "No Weathering Container", noWetGContainer,
					"Not weathering Gunpowdercontainer.");
			Property cupTex = cfg.get("render setting", "Set JPbowl Texture", setCupTexture,
					"Select the texture of the JP bowls." + "1:cherry flowers, 2:blue pattern, 3:white porcelain");
			...
			Property texPass = cfg.get("render setting", "Set Texture Type Number", setAltTexturePass,
					"Select the texture type number." + "1:default(x16 tex), 2:use x32 tex");
			Property cafeRecipe = cfg.get("entity setting", "New Villager ID", villagerRecipeID,
					"Set the number of new villager ID.");
			Property edibleEntity = cfg
					.get("render setting", "Enable Edible Entity", allowEdibleEntities,
			...
			Property mobBlock = cfg.get("setting", "Enable Mob Drop Container", enableMobBlock,
					"Enable to add some compression recipes of mob drop items." + BR
							+ "Rotten flesh, Bone, Spider eye, Ender parl, and Slime ball.");
			...
			Property noPvP = cfg.get("setting", "PvP Prohibition Mode", PvPProhibitionMode,
					"Disable the damage caused by items of this MOD against player." + BR
							+ "(For example, Yuzu Gatling, Silky Melon, etc.)");
			...
			Property dustDifP = cfg.get("difficulty setting", "JawCrusher Dust Gen", dustDif,
					"Change difficulty of the JawCrusher recipe." + BR + "0:sweet 1:normal 2:bitter 3:hard");

			Property chargeDifP = cfg.get("difficulty setting", "Battery Charge Gen", chargeDif,
					"Change difficulty of the battery charge amount." + BR + "0:sweet 1:normal 2:bitter 3:hard");
			...
			Property exchangeDifP = cfg.get("difficulty setting", "Exchange Rate of Charge", exchangeDif,
					"Change Rate of the charge exchange to the another energy." + BR
							+ "please set 0-4. smaller are fewer conversion, " + BR
							+ "and bigger are need more energy to exchange to AMT-Charge.");
			...
			useAltTeppanTex = useAltTeppan.getBoolean(false);
