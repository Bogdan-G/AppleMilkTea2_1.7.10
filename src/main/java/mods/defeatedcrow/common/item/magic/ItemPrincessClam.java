...
public class ItemPrincessClam extends Item {
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int posX,
		...
		} else if (meta == 3)// 風のチャーム
		{
			if (par3World.isRemote) {
				...
				if (par3World.isAirBlock(posX, posY + 1, posZ) && par3World.isAirBlock(posX, posY + 2, posZ)
						&& par3World.isSideSolid(posX, posY, posZ, ForgeDirection.UP))// 2ブロックの空間がある
				{
					...
					if (flag) {
						...
						nbt.setInteger("DCposX", posX);
						nbt.setInteger("DCposY", posY);
						nbt.setInteger("DCposZ", posZ);
						...
						par2EntityPlayer.addChatMessage(new ChatComponentText("Registered warp pos : " + posX + ", "
								+ (posY + 1) + ", " + posZ));
						return true;
					}
		...
		if (!world.isRemote && entityplayer instanceof EntityPlayerMP) {
			...
			if (meta == 3 && nbt != null)// wing
			{
				...
				if (mode == 2)// posモード
				{
					...
					if (thisPlayer.worldObj == world && world.provider.dimensionId == D
				...
				if (mode == 1)// playerモード
				{
					...
					if (target != null) {
						...
						for (int i = 0; i < 3; i++) {
							for (int k = 0; k < 3; k++) {
								if (world.isAirBlock(X + i - 1, Y, Z + k - 1)
										&& world.isAirBlock(X + i - 1, Y + 1, Z + k - 1)
						...
						if (flag) {
							...
							if (thisWorld && sameDim) {
								...
								thisPlayer.fallDistance = 0.0F;
								world.playSoundAtEntity(thisPlayer, "defeatedcrow:suzu", 1.0F, 1.2F);
								thisPlayer.addChatMessage(new ChatComponentText(
										"Succeeded to warp near the registered player : " + target.getDisplayName()));
								...
								thisPlayer.addChatMessage(new ChatComponentText(
										"Fail to warp near the registered player : " + target.getDisplayName()
												+ " is not alive in this dimention."));
							} else {
								thisPlayer.addChatMessage(new ChatComponentText(
										"Fail to get position of the registered player."));
							}
						} else {
							thisPlayer.addChatMessage(new ChatComponentText(
									"Fail to warp near the registered player : " + "Fail to get position of around "
	...
	public static boolean moonCanWarp(World world, int X, int Y, int Z) {
		boolean flag = false;
		if (Y > 250 || /* world.provider.hasNoSky || */world.getBlock(X, Y, Z) == null || world.isAirBlock(X, Y, Z))
			return false;
	...
	public boolean itemInteractionForEntity(ItemStack itemstack, EntityPlayer player, EntityLivingBase entity) {
		if (player.worldObj.isRemote || entity == null || itemstack == null || itemstack.getItemDamage() != 3) {
			return true;
		} else {
			NBTTagCompound nbt = itemstack.getTagCompound();
			int meta = itemstack.getItemDamage();
			...
			if (entity instanceof EntityPlayer && entity.isEntityAlive()) {
				...
				if (targetItem.getItem() == this) {
					...
					target.addChatMessage(new ChatComponentText(player.getDisplayName()
							+ " registered you to the Raden Charm"));
				}
